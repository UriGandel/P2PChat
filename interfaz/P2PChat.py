...
def actualizar_chat_box(chat_box, mensaje, root, nombre):
    ...
    chat_box.config(state=tk.DISABLED)
    notification.notify(
        title=f"Nuevo mensaje de {nombre}",
        message=f"{mensaje.split(':', 1)[0]}",
        app_name="P2P Chat",
        timeout=5
    )
    root.deiconify()
...
def resource_path(relative_path):
    """ Obtiene la ruta del archivo en el entorno de desarrollo o en el paquete generado. """
    if getattr(sys, 'frozen', False):
        # Si el script está en un paquete generado
...
icon_path = resource_path("icon-tk.png")
foto = tk.PhotoImage(file=icon_path)
